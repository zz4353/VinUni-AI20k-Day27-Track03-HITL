    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If your confidence is below 0.60, you MUST populate escalation_questions with "
                "2 to 4 specific, context-rich questions that reference the exact file and section "
                "in the diff that caused your uncertainty."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    qa = "\n".join(f"Q: {q}\nA: {ans}" for q, ans in state["escalation_answers"].items())
    llm = get_llm().with_structured_output(PRAnalysis)
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": "Refine your review using the additional context provided by the reviewer."},
            {"role": "user", "content": (
